...
const pluginSlug = "genesis-beta-tester";
...
const currentVersion = readmeContent.match(/Stable tag: ([\d.]+)/)[1];
...
if (!requiresMatch || !testedMatch || !requiresPhpMatch) {
  console.error(
    "Error: Could not find all required version information in readme.txt"
  );
...
const schema = JSON.parse(
  fs.readFileSync(path.join(__dirname, "info-json-schema.json"), "utf8")
);
...
function showDiff(downloadedInfo, outputPath) {
  ...
  try {
    const diff = execSync(
      `git diff --no-index "${tempPath}" "${outputPath}"`
    ).toString();
    if (diff) {
      ...
      console.log(
        "Did you remember to bump the plugin version in package.json?"
      );
    }
  } catch (error) {
...
async function createInfoJson() {
  try {
    ...
    const info = await fetchJson(
      `https://wpe-plugin-updates.wpengine.com/${pluginSlug}/info.json`
    );
    ...
    info.download_link = info.download_link.replace(
      /${pluginSlug}\.\d+\.\d+\.\d+\.zip/,
      `${pluginSlug}.${currentVersion}.zip`
    );
    ...
    // Some fields must be present to pass schema validation.
    if (!info?.sections?.screenshots) {
        info.sections.screenshots = "";
    }

    if (!info?.screenshots || info?.screenshots.length < 1) {
        info.screenshots = {};
    }

    if (!info?.banners || info?.banners.length < 1) {
